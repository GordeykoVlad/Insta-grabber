...
while True:
    ...
    variables = f'{{"id":"{user_id}","include_reel":true,"fetch_mutual":true,"first":80{after_value}}}'
    get_params = {
    ...
    with open(f'json/users/users{index}.json', 'r') as f:
        data = json.load(f)

    if not data['data']['user']['edge_followed_by']['page_info']['has_next_page']:
        break
    elif followers_in_progress == 1000:
        break
